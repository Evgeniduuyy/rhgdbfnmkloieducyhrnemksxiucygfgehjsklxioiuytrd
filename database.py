...
async def get_all_subscribers() -> list:
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        async with db.execute("""
            SELECT * FROM users
            WHERE subscription_lifetime = 1 OR (subscription_end IS NOT NULL AND subscription_end > datetime('now'))
...
async def add_session(session_data_encrypted: str) -> int:
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute(
            "INSERT INTO sessions (session_data, is_active) VALUES (?, 1)",
...
async def add_report_log(user_id: int, chat_id: str, message_id: str, success: int, total: int):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("""
            INSERT INTO report_logs (user_id, chat_id, message_id, success_count, total_count)
            VALUES (?, ?, ?, ?, ?)
...
async def add_payment(user_id: int, amount: float, days: int, invoice_id: str) -> int:
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute("""
            INSERT INTO payments (user_id, amount, duration_days, status, crypto_invoice_id)
            VALUES (?, ?, ?, 'pending', ?)
...
async def save_backup_bot(token_encrypted: str):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("DELETE FROM backup_bot")
        await db.execute(
            "INSERT INTO backup_bot (backup_bot_token, is_active) VALUES (?, 0)",
            (token_encrypted,)
        )
        await db.commit()
...
async def grant_subscription(user_id: int, days: int):
    """Добавляет days дней к текущей подписке (продлевает, если ещё активна)."""
    from datetime import timedelta
    user = await get_user(user_id)
    now = datetime.now(timezone.utc)
    if user and user.get("subscription_lifetime"):
        return  # у пожизненных ничего не меняем
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("""
            ...
            VALUES (?, '', '', ?, 0)
            ON CONFLICT(user_id) DO UPDATE SET subscription_end=excluded.subscription_end, subscription_lifetime=0
